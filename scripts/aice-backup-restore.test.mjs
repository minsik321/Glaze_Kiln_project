...
test("AiceRun logical data survives a clean migration backup and restore", async () => {
  ...
  try {
    const payload = { schema_version: 2, run_id: "backup-sample", revision: 4, title: "backup sample", status: "evaluated", goal: { gloss: "satin", transparency: "opaque" }, recipe: { id: "recipe-1" }, ware: { preset: "bowl" }, application: {}, thickness: {}, loading: {}, curves: {}, pid: {}, result: {}, sources: [{ source_type: "synthetic" }], consent: { share_allowed: true }, versions: { data: "data-2", rule_model: "aice-rule-rag-1", simulator: "aice-kiln-explanatory-1", predictor: null }, created_at: "2026-09-17T00:00:00Z", updated_at: "2026-09-17T00:00:00Z" };
    const { rows: [run] } = await source.query(`insert into public.aice_runs(title,payload,schema_version,status,goal_gloss,goal_transparency,recipe_id,ware_preset,is_public) values ('Backup sample',$1,2,'evaluated','satin','opaque','recipe-1','bowl',true) returning *`, [JSON.stringify(payload)]);
