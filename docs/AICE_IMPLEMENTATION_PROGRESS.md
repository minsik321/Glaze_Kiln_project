...
| 10. 검증·성능·로컬 실행 | 부분 완료 · 환경 차단 | `ad552b5` |
...
`cb3e200` (`phase-5: visualize evidence-aware glaze thickness`)
...
- 단열벽, 뚜껑, 문, 배기, 열선, 선반 3단을 구분한 전기가마 세로 종단면을
  만들고 선택한 기물 실루엣 여섯 개를 선반에 적재했다.
- 기본 1개, 상·중·하 3개, 다점 5개의 센서 프리셋과 센서별 위/아래 위치 조절을
  제공했다.
- 센서마다 측정 대상, 사각지대와 기물·유약 표면을 직접 측정하지 않는 대표성
  한계를 상세 보기에 기록했다.
- 중간 크기·여러 선반 적재 조건에 상·중·하 3개를 추천하고, 크기와 적재 수에
  따른 추천 규칙을 순수 함수로 분리했다.
- 예열, 승온, 유지, 냉각 구간 이동과 재생/일시정지, 시간 슬라이더를 만들고 같은
  시간축에 센서값, 기물 추정값, 층별 편차, 히터 출력을 연결했다.
- 냉색→온색 열지도와 느린 흐름선을 비정량 `설명용 근사`로 두고 합성 물리
  시계열과 별도 데이터 객체로 분리했다.
- 센서 편향·고장, 상단 과열, 층간 편차 시나리오를 추가하고 단면 위치와 경고
  타임라인을 동시에 강조했다.
- AiceRun 스냅샷에 선택 센서 수·위치·합성 온도·모델 버전·불확실성을 보존했다.

### 남은 작업

- 열·대류 표현은 CFD나 실제 가마 보정 결과가 아니며 실제 센서·제어 연결은
  의도적으로 포함하지 않았다.
- 합성 시계열의 제어 계획 비교와 PID 상세는 Phase 7에서 같은 시간축에 연결한다.

### 주요 설계 결정

- 설명용 온도 구배 계수는 `aice-kiln-explanatory-1` 합성 모델에만 속하며 실제
  가마 계수나 안전 한계로 사용하지 않는다.
- 센서 개수뿐 아니라 선반 층 커버리지와 선반에서의 거리를 불확실도에 반영한다.
- 센서 고장은 결측값으로 유지해 정상값으로 대체하지 않고 `판정 불가`로 표시한다.

### 주요 파일

- `app/react/aice/kilnSimulation.ts`, `kilnSimulation.test.ts`
- `app/react/aice/KilnSectionSimulator.tsx`, `KilnSectionSimulator.test.tsx`
- `app/react/aice/AicePrototype.tsx`, `design-system.css`
- `docs/baseline/phase-6-kiln-simulator.png`, `phase-6-kiln-controls.png`

### 테스트와 결과

- `npm run typecheck`: 통과
- `npm run test:react`: 31 passed
- `npm run build`: 통과
- `npm run test:e2e`: 4 passed
- 센서 모델 회귀: 센서 1개/3개의 관측값·불확실도 차이, 위치 이동에 따른
  재현 가능한 변화, 물리/시각 계층 분리, 고장 위치 경고를 검증했다.

### 실패와 수정

- 첫 E2E에서 새 SVG의 접근 가능한 제목이 기존 시나리오가 찾는 문구와 달라
  1건이 실패했다. 제목에 `가상 전기가마 종단면`을 명시하고 동일 4개 시나리오를
  다시 실행해 모두 통과했다.
- 독립 화면 캡처에서 Playwright 기본 Chromium 실행 파일이 없어 실패했다.
  프로젝트 E2E와 같은 설치된 Edge 채널을 사용해 두 화면을 캡처했다.

### 완료 커밋

`b866294` (`phase-6: simulate kiln sensing and heat`)

### 다음 Phase 시작점

Phase 7 TODO와 완료 조건을 다시 읽고 기준·두께 수정·시뮬레이션 실제·다음 제안
곡선을 한 시간축에 연결한다. 설명형 제어 프리셋과 고급 PID/포화/anti-windup
상세는 합성 모델임을 표시하고, 승인한 후보만 AiceRun에 기록한다.
...
- 기준 계획, 두께 반영 수정 계획, 시뮬레이션 실제, 다음 실행 제안을 하나의
  0–480분 그래프에 표시하고 각 역할을 체크박스로 켜고 끌 수 있게 했다.
- 도포 상태를 반영한 변경 구간에 직접 주석을 붙이고 변경 이유와 최고온도,
  유지시간, 총시간, 에너지 판정 불가를 자연어로 요약했다.
- 빠른 반응, 균형, 안정 우선의 설명형 제어 프리셋과 예상 효과를 먼저 제공했다.
- 계획값, 합성 센서값, 기물 추정값, 추종오차 경보와 히터 출력을 같은 시간축의
  결정론적 합성 샘플로 연결했다.
- 고급 상세에만 P/I/D 게인, 출력 포화, anti-windup, 샘플링 주기, 제약과 원시
  합성 로그를 표시했다.
- 후보 승인 전 AiceRun의 `selected_id`, PID 매개변수와 샘플은 비워 두고,
  승인 후에만 선택 곡선, 프리셋, 합성 게인·샘플·경보를 기록했다.

### 남은 작업

- 합성 제어기는 UI 설명과 소프트웨어 검증 전용이며 기존 Python 실제 계산 코어의
  `feedforward+P`를 대체하거나 실제 가마 PID 튜닝값을 제공하지 않는다.
- 실제 에너지 사용량은 계측 근거가 없어 계속 `판정 불가`로 둔다.

### 주요 설계 결정

- 세 프리셋의 P/I/D는 `synthetic` 출처와 실제 사용 금지 문구가 붙은 교육용
  계수다. 미정인 실제 가마 계수의 기본값으로 취급하지 않는다.
- 합성 제어기는 출력 포화 시 적분 누적을 멈추고, 승인 이벤트가 있어야만 결과를
  영속 스냅샷으로 내보낸다.
- 다음 실행 제안은 `inferred`, 나머지 비교·실행선은 `synthetic`으로 구분한다.

### 주요 파일

- `app/react/aice/curvePlan.ts`, `curvePlan.test.ts`
- `app/react/aice/CurveControlPanel.tsx`, `CurveControlPanel.test.tsx`
- `app/react/aice/AicePrototype.tsx`, `AicePrototype.test.tsx`
- `app/react/aice/design-system.css`
- `docs/baseline/phase-7-curve-control.png`, `phase-7-pid-details.png`

### 테스트와 결과

- `npm run typecheck`: 통과
- `npm run test:react`: 37 passed
- `npm run build`: 통과, 번들 500 kB 경고는 Phase 10 성능 점검 항목으로 기록
- `npm run test:e2e`: 4 passed
- 승인 회귀: 승인 전 선택/샘플 없음, 승인 후 선택 곡선·프리셋·합성 매개변수와
  샘플이 AiceRun에 기록됨을 검증했다.

### 실패와 수정

- 첫 컴포넌트 테스트에서 저장소에 없는 DOM 확장 matcher를 사용했고 닫힌
  `details`의 내용이 DOM에서 사라진다고 가정해 2건이 실패했다. 실제 checkbox
  속성과 `details.open` 상태를 검사하도록 고쳐 37개 전체를 통과했다.
- Phase 6 개발 서버의 자식 프로세스가 포트를 계속 점유했다. 포트의 정확한 PID를
  확인한 뒤 해당 Vite 프로세스만 종료하고 새 빌드로 E2E를 실행했다.
...
`ad552b5` (`phase-10: verify automated MVP boundaries`)는 자동 검증과 문서
체크포인트다. 실제 두 계정 수동 검증 전이므로 Phase 10 완료 커밋으로 간주하지
않는다.
